...
/** Isolated Postgres stress: only the SQL-backed endpoints (project page, version matrix, artifact
  * API) ramped to failure, to attribute saturation to the database and its joins specifically. Watch
  * `db_client_operation_duration_seconds` by `db_sql_table` in Grafana — especially
