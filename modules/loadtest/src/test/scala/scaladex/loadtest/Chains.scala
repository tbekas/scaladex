...
/** Reusable request chains, one per user-facing endpoint family, annotated with the backend each
  * one exercises. Each chain self-feeds so it can be composed freely.
  */
object Chains:

  // Elasticsearch-heavy (high fan-out) --------------------------------------
  val frontPage = // `/` fans out to ~6 ES queries
  ...
  val awesome = // `/awesome` fans out to ~3 ES queries
    exec(http("Awesome").get("/awesome").check(status.is(200)))

  // Postgres-heavy (sequential queries + big joins) -------------------------
  val projectPage = // project -> header -> dependencies -> dependents
  ...
  val versionMatrix = // getArtifactRefs — large scan
    feed(Feeders.projects)
