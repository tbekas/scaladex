...
running Scaladex and — together with the OpenTelemetry metrics already exported to the LGTM/Grafana
stack — shows **which endpoint is slow and whether the cost is in Postgres or Elasticsearch**.

- Client-side results: Gatling HTML report (percentiles, throughput, errors per request).
- Server-side breakdown: the **Scaladex Performance** Grafana dashboard + Tempo traces.

## 1. Run the app

The app auto-starts Postgres, Elasticsearch and the LGTM observability container via testcontainers.
...
(front/search pages read from Elasticsearch). The startup log also prints the Grafana port — open the
**Scaladex Performance** dashboard there.
